...
public class MeasurementScheduler extends Service{
      ...
      if (waitingTasksQueue.size()!=0) {
        ...
        if(ready.getDescription().endTime.before(new Date())){
          ...
          if (ready.getDescription().priority == MeasurementTask.USER_PRIORITY) {
            ...
            future = measurementExecutor.submit(new UserMeasurementTask(ready));
          } else {
            future = measurementExecutor.submit(new ServerMeasurementTask(ready));
          }
  ...
  private class UserMeasurementTask implements Callable<MeasurementResult[]> {
    ...
    private void broadcastMeasurementEnd(MeasurementResult result,String clientKey) {
      Intent intent = new Intent();
      intent.setAction(UpdateIntent.MEASUREMENT_PROGRESS_UPDATE_ACTION);
      ...
      if (result != null){
        if(result.getTaskProgress()==TaskProgress.PAUSED){
          ...
          intent.putExtra(UpdateIntent.RESULT_STRING_PAYLOAD, result.toString());
    ...
    public MeasurementResult[] call() throws MeasurementError {
      ...
      // TODO(Hongyi): why not catch exception and attach reason to result?
      finally {
  ...
  private class ServerMeasurementTask implements Callable<MeasurementResult []> {
    ...
    private void broadcastMeasurementEnd(MeasurementResult result, MeasurementError error, String clientKey) {
      ...
      if (!(error instanceof MeasurementSkippedException)) {
        ...
        if (result != null){
          if(result.getTaskProgress()==TaskProgress.PAUSED){
            ...
            // TODO(Hongyi): task is stopped, shall we return partial result?
            // Hongyi: currently just return null
            sendResultMessage(null, clientKey);
          }
        }
    ...
    public void handleMessage(Message msg) {
      ...
      switch (msg.what) {
        ...
        case Config.MSG_SUBMIT_TASK:
          ...
          MeasurementDesc desc = (MeasurementDesc) data.getParcelable("measurementDesc");
          if ( desc != null ) {
            switch (taskType) {
